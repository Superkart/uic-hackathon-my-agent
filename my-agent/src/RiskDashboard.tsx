...
interface MedRow { PATIENT: string; med_count: number }
interface DebtRow { PATIENTID: string; debt: number }
interface SdohRow { PATIENT: string; sdoh_count: number }
...
function scorePatient(
  ...
  if (s.ed_visits > 5)               { score += 3; factors.push(`${s.ed_visits} ED visits`); }
  if (!s.has_active_careplan)         { score += 2; factors.push("No care plan"); }
  if (s.chronic_condition_count > 10) { score += 2; factors.push(`${s.chronic_condition_count} conditions`); }
  if (medCount >= 5)                  { score += 1; factors.push(`Polypharmacy (${medCount})`); }
  if (debt > 10000)                   { score += 1; factors.push(`$${Math.round(debt / 1000)}k debt`); }
  if (sdohCount > 0)                  { score += Math.min(sdohCount, 2); factors.push(`${sdohCount} SDOH flag${sdohCount > 1 ? "s" : ""}`); }
...
const BADGE_VARIANT: Record<ScoredPatient["level"], "destructive" | "primary" | "secondary"> = {
...
function StatCard({ label, value, sub, icon, danger }: {
  label: string; value: string | number; sub?: string;
  icon: React.ReactNode; danger?: boolean;
}) {
  return (
    <Surface className={`p-4 rounded-xl ring ${danger ? "ring-kumo-danger" : "ring-kumo-line"} flex flex-col gap-1`}>
      <div className="flex items-center gap-1.5 text-kumo-inactive mb-1">
        {icon}
        <Text size="xs" variant="secondary">{label}</Text>
      </div>
      <span className={`text-2xl font-bold ${danger ? "text-kumo-danger" : "text-kumo-default"}`}>
        {value}
      </span>
      {sub && <Text size="xs" variant="secondary">{sub}</Text>}
    </Surface>
  );
}
  ...
  if (loading) {
    return (
      <div className="flex items-center gap-2 py-4 pl-12 text-kumo-inactive">
        ...
        <Text size="xs" variant="secondary">Loading patient profile...</Text>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-1.5">
            ...
            <Text size="xs" bold>Active Conditions ({detail.conditions.length})</Text>
          </div>
          <div className="space-y-1 max-h-40 overflow-y-auto">
            {detail.conditions.length === 0
              ? <Text size="xs" variant="secondary">None on record</Text>
              : detail.conditions.map((c, i) => (
                <div key={i} className="text-xs text-kumo-default bg-kumo-control rounded px-2 py-1">
                  {c.DESCRIPTION}
                </div>
              ))}
          </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-1.5">
            ...
            <Text size="xs" bold>
              ...
              {detail.meds.length >= 5 && <span className="ml-1 text-orange-500">⚠ Polypharmacy</span>}
            </Text>
          </div>
          <div className="space-y-1 max-h-40 overflow-y-auto">
            {detail.meds.length === 0
              ? <Text size="xs" variant="secondary">No active medications</Text>
              : detail.meds.map((m, i) => (
                <div key={i} className="text-xs text-kumo-default bg-kumo-control rounded px-2 py-1">
                  {m.DESCRIPTION}
                </div>
        ...
        <div className="space-y-3">
          <div className="space-y-2">
            <div className="flex items-center gap-1.5">
              ...
              <Text size="xs" bold>SDOH Signals</Text>
            </div>
            {detail.sdoh.length === 0
              ? <Text size="xs" variant="secondary">No PRAPARE flags</Text>
              : detail.sdoh.map((s, i) => (
                <div key={i} className="text-xs flex items-start gap-1.5">
                  <WarningIcon size={10} className="text-yellow-500 shrink-0 mt-0.5" weight="fill" />
          ...
          <div className="space-y-1 pt-2 border-t border-kumo-line">
            <div className="flex justify-between">
              <Text size="xs" variant="secondary">Outstanding debt</Text>
              <span className={`text-xs font-bold ${detail.debt > 10000 ? "text-kumo-danger" : "text-kumo-default"}`}>
                ${detail.debt.toLocaleString(undefined, { maximumFractionDigits: 0 })}
              </span>
            </div>
            <div className="flex justify-between">
              <Text size="xs" variant="secondary">Care plan</Text>
              {patient.has_active_careplan
                ? <span className="text-xs text-kumo-success flex items-center gap-1"><CheckCircleIcon size={10} /> Active</span>
                : <span className="text-xs text-kumo-danger flex items-center gap-1"><XCircleIcon size={10} /> None</span>}
            </div>
            <div className="flex justify-between">
              <Text size="xs" variant="secondary">Annual income</Text>
              <Text size="xs">${patient.income.toLocaleString()}</Text>
            </div>
            <div className="flex justify-between">
              <Text size="xs" variant="secondary">Race / ethnicity</Text>
              <Text size="xs">{patient.race} / {patient.ethnicity}</Text>
            </div>
          </div>
        </div>
      </div>
...
function PatientRow({ p, rank, expanded, expandedDetail, expandLoading, onToggle, onSendToAgent }: {
  ...
  return (
    <Surface className={`rounded-xl ring transition-all cursor-pointer ${expanded ? "ring-kumo-accent" : "ring-kumo-line hover:ring-kumo-accent/50"}`}>
      <div className="p-4" onClick={onToggle}>
        <div className="flex items-start gap-4">
          <div className="shrink-0 w-7 text-center">
            <span className={`text-lg font-bold ${rank <= 3 ? "text-kumo-danger" : "text-kumo-inactive"}`}>#{rank}</span>
          </div>
          <div className="flex-1 min-w-0 space-y-2">
            <div className="flex items-center gap-2 flex-wrap">
              <Text size="sm" bold>{p.name}</Text>
              <Badge variant={BADGE_VARIANT[p.level]}>{p.level.toUpperCase()}</Badge>
              <Text size="xs" variant="secondary">{p.age}y · {p.gender === "M" ? "M" : "F"} · {p.race} · {p.city}</Text>
            </div>
            <div className="flex items-center gap-3">
              <div className="flex-1 bg-kumo-control rounded-full h-2 overflow-hidden">
                <div className={`h-2 rounded-full transition-all duration-500 ${LEVEL_BAR[p.level]}`} style={{ width: `${barPct}%` }} />
              </div>
              <span className={`text-sm font-bold shrink-0 ${LEVEL_COLOR[p.level]}`}>{p.score}/14</span>
            </div>
            <div className="flex flex-wrap gap-1">
              {p.factors.map((f) => (
                <span key={f} className="text-xs px-2 py-0.5 rounded-full bg-kumo-control border border-kumo-line text-kumo-default">{f}</span>
              ))}
            </div>
          </div>
          <div className="shrink-0 text-right space-y-1 mr-2">
            <div className="flex items-center gap-1 justify-end text-kumo-danger">
              ...
              <Text size="xs" bold>{p.ed_visits} ED</Text>
            </div>
            <Text size="xs" variant="secondary">{p.chronic_condition_count} conditions</Text>
            <Text size="xs" variant="secondary">${Math.round(p.ed_cost / 1000)}k ED cost</Text>
          </div>
...
export function RiskDashboard({ onSendToAgent }: RiskDashboardProps) {
  ...
  const [expandCache, setExpandCache] = useState<Map<string, ExpandedDetail>>(new Map());
  ...
  const load = async () => {
    ...
    try {
      ...
      const scored: ScoredPatient[] = summaries.map((s) => {
        ...
        return {
          id: s.id, name: `${s.first} ${s.last}`,
          age: s.age, gender: s.gender, race: s.race,
          ethnicity: s.ethnicity, income: s.income,
          city: s.city, state: s.state,
          ed_visits: s.ed_visits, inpatient_visits: s.inpatient_visits,
          total_visits: s.total_visits, chronic_condition_count: s.chronic_condition_count,
          has_active_careplan: !!s.has_active_careplan,
          total_cost: s.total_cost, ed_cost: s.ed_inpatient_total_cost,
          debt, med_count: medCount, sdoh_flags: sdohCount, score, level, factors
        };
      });
      setPatients(scored);
    } catch (e) {
  ...
  const toggleExpand = useCallback(async (p: ScoredPatient) => {
    ...
    try {
      ...
      const detail: ExpandedDetail = { conditions, meds, careplans, sdoh, debt: debtRows[0]?.debt ?? 0 };
  ...
  const actionableEdCost = patients.filter((p) => p.level === "critical" || p.level === "high").reduce((s, p) => s + p.ed_cost, 0);
  ...
  const raceEquity = [...raceMap.entries()]
    .map(([race, d]) => ({ race, ...d, pct: Math.round((d.highRisk / d.total) * 100) }))
      ...
      <div className="px-5 py-4 bg-kumo-base border-b border-kumo-line">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-semibold text-kumo-default">Population Risk Dashboard</h2>
            {!loading && <Badge variant="secondary">{patients.length} patients</Badge>}
          </div>
          ...
          {!loading && patients.length > 0 && (
            <>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <StatCard label="Critical Risk" value={critical} sub="Immediate action needed"
                  icon={<WarningIcon size={14} weight="fill" />} danger />
                <StatCard label="High Risk" value={high} sub="Monitor closely"
                  icon={<WarningIcon size={14} />} />
                <StatCard label="No Care Plan" value={noCarePlan} sub="Biggest gap"
                  icon={<XCircleIcon size={14} />} />
                <StatCard label="Preventable ED Cost" value={`$${Math.round(actionableEdCost / 1000)}k`}
                  sub="Critical + high patients" icon={<CurrencyDollarIcon size={14} />} danger />
              </div>
              ...
              {actNow.length > 0 && (
                <Surface className="p-4 rounded-xl ring-2 ring-kumo-danger space-y-3">
                  <div className="flex items-center gap-2">
                    <WarningIcon size={16} weight="fill" className="text-kumo-danger" />
                    <Text size="sm" bold>Act Now — Critical patients with no care plan</Text>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    {actNow.map((p) => (
                      <div key={p.id}
                        className="p-3 rounded-lg bg-kumo-control border border-kumo-danger/30 space-y-2 cursor-pointer hover:border-kumo-danger transition-colors"
                        onClick={() => toggleExpand(p)}>
                        <div className="flex justify-between items-start">
                          <Text size="xs" bold>{p.name}</Text>
                          <span className="text-xs font-bold text-kumo-danger">{p.score}/14</span>
                        </div>
                        <div className="flex flex-wrap gap-1">
                          {p.factors.slice(0, 3).map((f) => (
                            <span key={f} className="text-xs px-1.5 py-0.5 rounded bg-kumo-danger/10 text-kumo-danger border border-kumo-danger/20">{f}</span>
                          ))}
                        </div>
                        <Button variant="primary" size="sm" icon={<ChatCircleDotsIcon size={12} />}
                          onClick={(e) => { e.stopPropagation(); onSendToAgent(p.name, p.id); }}>
                          Analyze
                        </Button>
                      </div>
                    ))}
              ...
              <Surface className="p-4 rounded-xl ring ring-kumo-line space-y-3">
                <Text size="sm" bold>Risk Distribution — All {patients.length} Patients</Text>
                <div className="flex rounded-full overflow-hidden h-5">
                  {(["critical", "high", "medium", "low"] as ScoredPatient["level"][]).map((lvl) => {
                    const count = patients.filter((p) => p.level === lvl).length;
                    const pct = (count / patients.length) * 100;
                    return (
                      <div key={lvl} title={`${lvl}: ${count}`}
                  ...
                  {(["critical", "high", "medium", "low"] as ScoredPatient["level"][]).map((lvl) => {
                    ...
                    return (
                      <div key={lvl} className="flex items-center gap-1.5">
                        <div className={`w-2.5 h-2.5 rounded-full ${LEVEL_BAR[lvl]}`} />
                        <Text size="xs" variant="secondary">
                          {lvl.charAt(0).toUpperCase() + lvl.slice(1)}: {count} ({pct}%)
                        </Text>
                      </div>
                    );
                  })}
              ...
              <Surface className="p-4 rounded-xl ring ring-kumo-line space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <Text size="sm" bold>Health Equity — High-Risk Rate by Race</Text>
                </div>
                <div className="space-y-2">
                  {raceEquity.map(({ race, total, highRisk, pct }) => (
                    <div key={race} className="flex items-center gap-3">
                      <span className="text-xs text-kumo-default w-16 shrink-0 capitalize">{race}</span>
                      <div className="flex-1 bg-kumo-control rounded-full h-2.5 overflow-hidden">
                        <div className={`h-2.5 rounded-full ${pct >= 30 ? "bg-kumo-danger" : pct >= 15 ? "bg-orange-500" : "bg-kumo-success"}`}
                          style={{ width: `${pct}%` }} />
                      </div>
                ...
                <Text size="xs" variant="secondary">
                  ≥30% high-risk rate may indicate systemic care gap for that group
                </Text>
              </Surface>
              ...
              <Surface className="p-4 rounded-xl ring ring-kumo-line">
                <div className="flex items-center gap-2 mb-3">
                  <CurrencyDollarIcon size={16} className="text-kumo-inactive" />
                  <Text size="sm" bold>Cost Reduction Opportunity</Text>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  <div>
                    <Text size="xs" variant="secondary">Total ED/inpatient (all patients)</Text>
                    <span className="text-lg font-bold text-kumo-default">${Math.round(totalEdCost).toLocaleString()}</span>
                  </div>
                  <div>
                    <Text size="xs" variant="secondary">From critical + high patients</Text>
                    <span className="text-lg font-bold text-kumo-danger">${Math.round(actionableEdCost).toLocaleString()}</span>
                  </div>
                  <div>
                    <Text size="xs" variant="secondary">Est. savings at 30% prevention</Text>
                    <span className="text-lg font-bold text-kumo-success">${Math.round(actionableEdCost * 0.3).toLocaleString()}</span>
                  </div>
                </div>
              </Surface>
              ...
              <div className="flex flex-wrap gap-2 items-center justify-between">
                <div className="flex gap-2 flex-wrap">
                  {(["all", "critical", "high", "medium", "low"] as (ScoredPatient["level"] | "all")[]).map((lvl) => (
                    <button key={lvl} type="button" onClick={() => setFilterLevel(lvl)}
                      ...
                      {lvl === "all" ? `All (${patients.length})` : `${lvl.charAt(0).toUpperCase() + lvl.slice(1)} (${patients.filter((p) => p.level === lvl).length})`}
                    </button>
                  ))}
                </div>
                <div className="flex gap-1 flex-wrap">
                  {(Object.keys(SORT_LABELS) as SortKey[]).map((key) => (
                    <button key={key} type="button" onClick={() => toggleSort(key)}
                      ...
                      {SORT_LABELS[key]}
                      {sortKey === key && (sortAsc ? <ArrowUpIcon size={10} /> : <ArrowDownIcon size={10} />)}
                    </button>
                  ))}
                </div>
              </div>
